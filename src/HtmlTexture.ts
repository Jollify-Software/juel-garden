import { DynamicTexture, Matrix } from "babylonjs";
...
export class HTMLTexture {
    ...
    stateHoverElements: HTMLElement[];
    stateFocusedElements: HTMLElement[];
    focusedElement: HTMLElement;
    cssgenerated = []; // Remeber what css sheets have already been passed
    cssembed = []; // The text of the css to included in the SVG to render

    constructor(private el: GardenMesh, private textureResolution: number = 512/*, updateCallback, eventCallback*/) {
      
      //this.updateCallback = updateCallback;
      //this.eventCallback = eventCallback;
  
      // Create the canvas to be drawn to
      this.texture = new DynamicTexture("htmlTexture", textureResolution, el.getScene(), false);
      ...
      this.el.addEventListener('mousemove', this.mousemovehtml);
      ...
      var callback = (mutationsList: MutationRecord[], observer: MutationObserver) => {
        // Don't update if we are manipulating DOM for render
        if (this.nowatch) return;
  
        for (var i = 0; i < mutationsList.length; i++) {
          let target = mutationsList[i].target as any;
          // Skip the emebed html element if attributes change
          if (target == this.el && mutationsList[i].type == "attributes") continue;
    ...
    forceRender() {
      ...
      //Array.from(document.querySelectorAll('*')).map((ele: HTMLElement) => ele.classCache = {});
   ...
   //   this.el.addEventListener('mousemove', this.mousrmovehtml);
    }
    ...
    embedCss(url, css): Promise<string> {
    ...
    getDataURL(url) {
      return new Promise(resolve => {
        this.getURL(url).then((xhr: XMLHttpRequest) => {
    ...
    embededSVG() {
      var promises = [];
      var elements = this.el.querySelectorAll("*") as NodeListOf<HTMLElement>;
      for (var i = 0; i < elements.length; i++) {
        ...
        if (elements[i].tagName == "IMG" && (<HTMLImageElement>elements[i]).src.substr(0, 4) != "data") {
          promises.push(this.getDataURL((<HTMLImageElement>elements[i]).src).then(((element) => {
            return dataURL => {
      ...
      var styles = this.el.querySelectorAll("style");
      for (var i = 0; i < styles.length; i++) {
        promises.push(
    ...
    updateFocusBlur() {
      var allElements = this.el.querySelectorAll("*") as NodeListOf<HTMLElement>;
        ...
        if (element.tabIndex > -1) {
          ...
          if (!element.hasOwnProperty('blur')) {
           // element.blur = ((element) => {
            //  return () => this.focusedElement == element ? this.setBlur() : false;
           // })(element)
          }
        } else {
    ...
    updateCheckedAttributes() {
      var inputElements = this.el.getElementsByTagName("input");
    ...
    svgToImg() {
      ...
      Promise.all([this.embededSVG(), this.generatePageCSS()]).then(() => {
        ...
        var docString = this.serialiser.serializeToString(this.el);
    ...
    */
    ...
    checkElement(x, y, offsetx, offsety, offsetz, level, element, result) {
      ...
      if (style['position'] != 'static' && element != this.el) {
    ...
    elementAt(x, y) {
      ...
      this.checkElement(x, y, 0, 0, 0, 0, this.el, result);
      this.el.style.display = 'none';
      return result.ele;
    }
  /*
